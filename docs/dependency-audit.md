...
Last updated: 2026-09-22 (issue #522 — Dioxus now resolves from crates.io, not a git tag; previously issues #2054, #1767, #1710, #1621, #1529, #1530, #1100, #1268, #1147, #643).
...
| `const-serialize` | 0.7.2, 0.8.0-alpha.0 | Both via `manganis`/`manganis-core` 0.7.9, which deliberately depend on both: 0.7.2, `=`-pinned under the rename `const-serialize-07`, beside 0.8.0-alpha.0 | **blocked by upstream** | Not a lag — manganis 0.7.x deliberately carries both serialization formats. Still present after the switch to crates.io (issue #522), and 0.7.10 keeps the same pair. Collapses only when manganis drops the 0.7 format. |
...
| `axum-extra` | 0.10.3, 0.12.6 | 0.10.3 via `dioxus-fullstack` 0.7.9; 0.12.6 via our own `omnibus` server crate directly | **blocked by upstream** | Mirrors the `tower-http` split below: Dioxus hasn't caught up to the `axum-extra` version our server crate uses (0.7.10 still asks for `^0.10.1`). Collapses on a Dioxus release that moves to 0.12. |
...
| `reqwest` | 0.12.28, 0.13.4 | 0.12.28 via `dioxus-fullstack` 0.7.9; 0.13.4 via `omnibus-db`, `omnibus-frontend`, `omnibus-mobile` — the workspace-pinned version per `Cargo.toml`'s `[workspace.dependencies] reqwest = { version = "0.13", ... }` | **blocked by upstream** | Two-*major*-version split, the largest gap in this table (the rest is minor-version churn). Exists because `dioxus-fullstack` hasn't caught up to the `reqwest 0.13` the workspace standardized on (0.7.10 still asks for `^0.12.28`). Should collapse on a Dioxus release that moves to 0.13, same caveat as the `tungstenite`/`const-serialize` rows. |
| `gloo-net` | 0.6.0, 0.7.0 | 0.6.0 via `dioxus-fullstack` 0.7.9; 0.7.0 via `omnibus-frontend`'s own direct pin (`frontend/Cargo.toml`, behind the `web` feature) | **blocked by upstream** | Same "Dioxus version lag" shape as `axum-extra`/`tower-http`/`reqwest` above: Dioxus hasn't caught up to the `gloo-net` version the WASM frontend pins directly (0.7.10 still asks for `^0.6.0`). Moving to crates.io (#522) did not collapse it; a Dioxus release on `gloo-net` 0.7 will. |
| `gloo-timers` | 0.3.0, 0.4.0 | 0.3.0 via `dioxus-web` 0.7.9; 0.4.0 via `omnibus-frontend`'s own direct pin (`frontend/Cargo.toml`, behind the `web` feature) | **blocked by upstream** | Same root cause as the `gloo-net` row above, and collapses the same way. |
| `gloo-utils` | 0.2.0, 0.3.0 | 0.2.0 transitively via `gloo-net 0.6.0`; 0.3.0 transitively via `gloo-net 0.7.0` | **blocked by upstream** | Follows the `gloo-net` split one level down and collapses in lockstep with it. |
...
- `lru 0.16.4` — RUSTSEC-2026-0253 (unsound: use-after-free in
  ...
  via `dioxus-server` 0.7.9 → `dioxus`, which asks for `lru ^0.16` (0.7.10
  too), so it is not independently bumpable. Accepted — exploiting it requires
...
- **Bumping Dioxus:** re-run this audit. Bump the `=` pins in the root `Cargo.toml` together with the `dx` CLI (flake.nix and the Dockerfile), since `dx` flags any other `dioxus` version as incompatible. The websocket cluster will be the first to collapse; const-serialize is by design in manganis 0.7.x.
