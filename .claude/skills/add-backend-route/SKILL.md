...
- **Binary upload (file/multipart):** server functions JSON-serialize their args, so they can't carry a file. Use a hand-written `/api/*` REST handler with `axum::extract::Multipart` for **both** web and mobile, and have the web `data/` helper POST directly via `gloo-net` + `web_sys::FormData` (see `backend/uploads.rs` + `data/uploads.rs`, modeled on the author-photo upload). Large uploads need their own sub-router with a bigger `DefaultBodyLimit`, merged after the route-level `REQUEST_TIMEOUT` layer with a `RequestBodyTimeoutLayer` idle timeout in its place (see `book_upload_router` and `rest_router_with_timeouts`) — a body that takes more than 30 s to arrive otherwise gets a 408; if the handler then triggers a reindex, `worker.post(Task::Scan{..})` + `await_completion` before reading the result back.
...
- A refusal the web client must branch on travels in `ServerFnError::ServerError`'s `code`, with `details: None` — never in `details`. Under `Result<T>` the client decodes `details` *as* the whole `CapturedError`, dropping the code and the message. `CrossFormatErrorCode` + `classify_sync_point_err` are the worked example; `rpc/cross_format/tests.rs` round-trips through Dioxus's real encoder and decoder.
