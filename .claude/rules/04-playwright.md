...
The JPEG 2000 decode test in `pdf_reader.spec.ts` opens `flatland`'s route but
**intercepts its `/file` response** with the two-page JPX-only PDF that
`tests/fixtures/jpx_pdf.ts` builds in memory — the seeded PDFs carry no JPX,
and a third library fixture would ripple through `FIXTURE_BOOKS` and every
exact-count assertion. It writes nothing (page 1 of 2 is never the end, and
`flatland` is already `reading`), so the pristine contract holds.
