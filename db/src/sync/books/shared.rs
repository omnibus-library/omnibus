...
use crate::normalize::{creator_sort_key, normalize_author, normalize_title};
...
async fn update_book_row(
    ...
    let author_sort = primary_author_sort(m);
...
/// `books.author_sort` for `m`: the first creator's surname-first key. Neither
/// a bare display name nor a comma-less OPF `file_as` (`"Andy Weir"`) is
/// stored verbatim, or that book files apart from the author's others.
fn primary_author_sort(m: &EbookMetadata) -> Option<String> {
    m.creators
        .first()
