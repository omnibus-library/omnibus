...
#[tokio::test]
async fn book_progress_resolves_a_pdf_page_against_the_pdf_on_a_mixed_book() {
    // The book carries an EPUB (the text source) and a PDF; a pdf-page
    // anchor must read the PDF's rows, and a page past its end places
    // nothing rather than a confident position past the document.
    let pool = init_db("sqlite::memory:").await.unwrap();
    let user = seed_user(&pool, "alice").await;
    let (book_id, uuid) = seed(&pool, "/lib", "Mixed").await;
    seed_epub_structure(&pool, book_id).await;
    let pdf_file_id: i64 = sqlx::query_scalar(
        "INSERT INTO book_files (book_id, format, filename, size_bytes) \
         VALUES (?, 'PDF', 'mixed', 0) RETURNING id",
    )
    .bind(book_id)
    .fetch_one(&pool)
    .await
    ...
    .await
    .unwrap();
    for (page, expect_resolved) in [(1usize, true), (7usize, false)] {
        upsert_progress(
            &pool,
            user,
            &ProgressUpdate {
                book_uuid: uuid.clone(),
                format: ProgressFormat::Epub,
                epub_cfi: Some(omnibus_shared::pdf_page_anchor(page)),
                audio_position_seconds: None,
                progress_percent: Some(50),
                kobo_location: None,
                book_file_id: None,
                client_updated_at: Some(100 + page as i64),
            },
        )
        .await
        .unwrap();
        let progress = book_progress(&pool, user, &uuid, None)
            ...
            .unwrap()
            .unwrap();
        let resolved = progress.records[0].resolved.as_ref();
        if expect_resolved {
            let resolved = resolved.expect("page 1 of 2 resolves");
            assert_eq!(
                resolved.chapter_title.as_deref(),
                Some("Scan"),
                "the PDF's outline, not the EPUB's TOC"
            );
            assert_eq!(resolved.chapters_total, Some(1));
            assert_eq!(resolved.percent_through_book, Some(100));
        } else {
            assert!(resolved.is_none(), "page 7 of 2 places nothing");
        }
    }
}
